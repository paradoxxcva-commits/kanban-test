import { createFileRoute, Link } from "@tanstack/react-router";
import { useMemo, useState } from "react";
import { useQuery } from "@tanstack/react-query";
import { AppShell } from "@/components/layout/app-shell";
import { listTasksInRange, getOrCreateCalendarToken, revokeCalendarToken, getActiveCalendarToken } from "@/lib/calendar-api";
import { listBoards, listColumns, type ColumnRow } from "@/lib/boards-api";
import { TaskDialog } from "@/components/boards/task-dialog";
import type { TaskRow } from "@/lib/boards-api";
import { useAuth } from "@/lib/auth-context";
import { ChevronLeft, ChevronRight, Link2, Copy, RotateCcw, CalendarDays, Plus } from "lucide-react";
import { Button } from "@/components/ui/button";
import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogHeader,
  DialogTitle,
} from "@/components/ui/dialog";
import { toast } from "sonner";

export const Route = createFileRoute("/_authenticated/calendar")({
  head: () => ({ meta: [{ title: "Календарь — Канбан" }] }),
  component: CalendarPage,
});

const WEEKDAYS = ["Пн", "Вт", "Ср", "Чт", "Пт", "Сб", "Вс"];

function startOfMonth(d: Date) { return new Date(d.getFullYear(), d.getMonth(), 1); }
function endOfMonth(d: Date) { return new Date(d.getFullYear(), d.getMonth() + 1, 1); }
function startOfGrid(d: Date) {
  const s = startOfMonth(d);
  // Monday-first: weekday 1..7 where Mon=1
  const dow = (s.getDay() + 6) % 7;
  return new Date(s.getFullYear(), s.getMonth(), s.getDate() - dow);
}
function addDays(d: Date, n: number) { return new Date(d.getFullYear(), d.getMonth(), d.getDate() + n); }
function sameDay(a: Date, b: Date) {
  return a.getFullYear() === b.getFullYear() && a.getMonth() === b.getMonth() && a.getDate() === b.getDate();
}

const PRIORITY_DOT: Record<string, string> = {
  low: "bg-muted-foreground/50",
  normal: "bg-brand",
  high: "bg-amber-500",
  urgent: "bg-destructive",
};

function CalendarPage() {
  const { user } = useAuth();
  const [cursor, setCursor] = useState(() => new Date());
  const [icalOpen, setIcalOpen] = useState(false);
  const [taskOpen, setTaskOpen] = useState(false);
  const [editingTask, setEditingTask] = useState<TaskRow | null>(null);
  const [createDate, setCreateDate] = useState<Date | null>(null);
  const [createBoardId, setCreateBoardId] = useState<string>("");
  const [createColumns, setCreateColumns] = useState<ColumnRow[]>([]);

  const monthStart = startOfMonth(cursor);
  const monthEnd = endOfMonth(cursor);
  const gridStart = startOfGrid(cursor);
  const gridEnd = addDays(gridStart, 42);

  const days = useMemo(() => Array.from({ length: 42 }, (_, i) => addDays(gridStart, i)), [gridStart]);

  const tasksQ = useQuery({
    queryKey: ["calendar-tasks", gridStart.toISOString(), gridEnd.toISOString()],
    queryFn: () => listTasksInRange(gridStart, gridEnd),
  });

  const boardsQ = useQuery({ queryKey: ["boards"], queryFn: listBoards });

  const tasksByDay = useMemo(() => {
    const map = new Map<string, TaskRow[]>();
    for (const t of tasksQ.data ?? []) {
      if (!t.due_date) continue;
      const d = new Date(t.due_date);
      const key = `${d.getFullYear()}-${d.getMonth()}-${d.getDate()}`;
      const list = map.get(key) ?? [];
      list.push(t);
      map.set(key, list);
    }
    return map;
  }, [tasksQ.data]);

  const openCreateForDay = async (day: Date) => {
    const boards = boardsQ.data ?? [];
    if (!boards.length) {
      toast.error("Нет ни одной доски — создайте доску, чтобы добавить задачу");
      return;
    }
    const boardId = boards[0].id;
    const cols = await listColumns(boardId);
    if (!cols.length) {
      toast.error("В доске нет колонок");
      return;
    }
    setCreateDate(day);
    setCreateBoardId(boardId);
    setCreateColumns(cols);
    setEditingTask({
      id: "",
      board_id: boardId,
      column_id: cols[0].id,
      title: "",
      description: null,
      assignee_id: null,
      due_date: day.toISOString(),
      completed_at: null,
      position: 0,
      priority: "normal",
      created_by: user?.id ?? null,
      created_at: "",
      updated_at: "",
    } as TaskRow);
    // We actually want creation flow, not edit. Use create path:
    setEditingTask(null);
    setTaskOpen(true);
  };

  const openEditTask = async (t: TaskRow) => {
    const cols = await listColumns(t.board_id);
    setCreateBoardId(t.board_id);
    setCreateColumns(cols);
    setEditingTask(t);
    setCreateDate(t.due_date ? new Date(t.due_date) : null);
    setTaskOpen(true);
  };

  const monthLabel = cursor.toLocaleDateString("ru-RU", { month: "long", year: "numeric" });

  return (
    <AppShell>
      <div className="mx-auto max-w-7xl space-y-4 p-6 lg:p-8">
        <header className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <div className="flex items-center gap-2 text-xs uppercase tracking-wider text-muted-foreground">
              <CalendarDays className="h-3.5 w-3.5" /> Календарь
            </div>
            <h1 className="mt-1 text-2xl font-semibold tracking-tight text-foreground capitalize">
              {monthLabel}
            </h1>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() - 1, 1))}
              className="ring-focus rounded-md border border-border bg-surface p-2 hover:bg-accent"
              aria-label="Предыдущий месяц"
            >
              <ChevronLeft className="h-4 w-4" />
            </button>
            <button
              onClick={() => setCursor(new Date())}
              className="ring-focus rounded-md border border-border bg-surface px-3 py-1.5 text-xs hover:bg-accent"
            >
              Сегодня
            </button>
            <button
              onClick={() => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() + 1, 1))}
              className="ring-focus rounded-md border border-border bg-surface p-2 hover:bg-accent"
              aria-label="Следующий месяц"
            >
              <ChevronRight className="h-4 w-4" />
            </button>
            <Button variant="outline" size="sm" onClick={() => setIcalOpen(true)}>
              <Link2 className="h-4 w-4" /> iCal-фид
            </Button>
          </div>
        </header>

        <div className="surface-card overflow-hidden">
          <div className="grid grid-cols-7 border-b border-border bg-surface">
            {WEEKDAYS.map((d) => (
              <div key={d} className="px-2 py-2 text-center text-[11px] font-medium uppercase tracking-wider text-muted-foreground">
                {d}
              </div>
            ))}
          </div>
          <div className="grid grid-cols-7">
            {days.map((day, idx) => {
              const inMonth = day >= monthStart && day < monthEnd;
              const isToday = sameDay(day, new Date());
              const key = `${day.getFullYear()}-${day.getMonth()}-${day.getDate()}`;
              const tasks = tasksByDay.get(key) ?? [];
              return (
                <div
                  key={idx}
                  className={`group relative min-h-[110px] border-b border-r border-border p-1.5 ${
                    inMonth ? "bg-background" : "bg-surface/40"
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <span
                      className={`inline-flex h-6 min-w-6 items-center justify-center rounded-full px-1 text-xs ${
                        isToday
                          ? "bg-brand font-semibold text-brand-foreground"
                          : inMonth
                            ? "text-foreground"
                            : "text-muted-foreground/60"
                      }`}
                    >
                      {day.getDate()}
                    </span>
                    <button
                      onClick={() => openCreateForDay(day)}
                      className="opacity-0 transition-opacity hover:text-brand group-hover:opacity-100"
                      aria-label="Добавить задачу"
                    >
                      <Plus className="h-3.5 w-3.5" />
                    </button>
                  </div>
                  <div className="mt-1 space-y-1">
                    {tasks.slice(0, 3).map((t) => (
                      <button
                        key={t.id}
                        onClick={() => openEditTask(t)}
                        className="flex w-full items-center gap-1.5 truncate rounded bg-accent/60 px-1.5 py-1 text-left text-[11px] text-foreground hover:bg-accent"
                        title={t.title}
                      >
                        <span className={`h-1.5 w-1.5 shrink-0 rounded-full ${PRIORITY_DOT[t.priority ?? "normal"] ?? PRIORITY_DOT.normal}`} />
                        <span className={`truncate ${t.completed_at ? "line-through text-muted-foreground" : ""}`}>
                          {t.title}
                        </span>
                      </button>
                    ))}
                    {tasks.length > 3 && (
                      <div className="px-1.5 text-[10px] text-muted-foreground">+ ещё {tasks.length - 3}</div>
                    )}
                  </div>
                </div>
              );
            })}
          </div>
        </div>

        <p className="text-xs text-muted-foreground">
          Чтобы создать доску и задачи, перейдите в раздел{" "}
          <Link to="/boards" className="text-brand hover:underline">Доски</Link>.
        </p>
      </div>

      {taskOpen && createColumns.length > 0 && (
        <TaskDialog
          open={taskOpen}
          onOpenChange={(v) => {
            setTaskOpen(v);
            if (!v) {
              tasksQ.refetch();
              setEditingTask(null);
            }
          }}
          boardId={createBoardId}
          columns={createColumns}
          initialColumnId={createColumns[0]?.id}
          task={editingTask}
          key={(editingTask?.id ?? "new") + (createDate?.toISOString() ?? "")}
        />
      )}

      <IcalDialog open={icalOpen} onOpenChange={setIcalOpen} userId={user?.id ?? null} />
    </AppShell>
  );
}

function IcalDialog({ open, onOpenChange, userId }: { open: boolean; onOpenChange: (v: boolean) => void; userId: string | null }) {
  const tokenQ = useQuery({
    queryKey: ["calendar-token", userId],
    queryFn: () => (userId ? getActiveCalendarToken(userId) : Promise.resolve(null)),
    enabled: !!userId && open,
  });

  const onCreate = async () => {
    if (!userId) return;
    await getOrCreateCalendarToken(userId);
    tokenQ.refetch();
  };
  const onRevoke = async () => {
    if (!tokenQ.data) return;
    if (!confirm("Сбросить текущий токен? Старая ссылка перестанет работать.")) return;
    await revokeCalendarToken(tokenQ.data.id);
    tokenQ.refetch();
  };

  const url = tokenQ.data
    ? `${typeof window !== "undefined" ? window.location.origin : ""}/api/public/ical/${tokenQ.data.token}`
    : "";

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>iCal-фид</DialogTitle>
          <DialogDescription>
            Защищённая ссылка для подписки в Яндекс.Календаре, Google Calendar или Apple. Включает все задачи ваших досок со сроком.
          </DialogDescription>
        </DialogHeader>
        {tokenQ.data ? (
          <div className="space-y-3">
            <div className="rounded-md border border-border bg-surface p-2 text-xs font-mono break-all text-foreground">
              {url}
            </div>
            <div className="flex flex-wrap gap-2">
              <Button
                size="sm"
                variant="outline"
                onClick={async () => {
                  await navigator.clipboard.writeText(url);
                  toast.success("Ссылка скопирована");
                }}
              >
                <Copy className="h-4 w-4" /> Скопировать
              </Button>
              <Button size="sm" variant="ghost" onClick={onRevoke}>
                <RotateCcw className="h-4 w-4" /> Сбросить и создать новую
              </Button>
            </div>
            <p className="text-xs text-muted-foreground">
              В Яндекс.Календаре: «Добавить календарь → По ссылке» — вставьте URL.
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">Ссылка ещё не создана.</p>
            <Button size="sm" onClick={onCreate}>
              <Link2 className="h-4 w-4" /> Создать ссылку
            </Button>
          </div>
        )}
      </DialogContent>
    </Dialog>
  );
}
